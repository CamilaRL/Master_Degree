...
temp6, QFI_q6 = np.loadtxt(f'./Results/QFI_q6_g{g:.2f}_ttherm{tSEmax:.3f}.txt', unpack=True)
...
plt.plot(temp2, QFI_q2, color='green', linestyle='-', label=f'Qubit 2 - TMax = {temp2[np.argmax(QFI_q2)]:.3f}')
plt.plot(temp3, QFI_q3, color='orange', linestyle='--', label=f'Qubit 3 - TMax = {temp3[np.argmax(QFI_q3)]:.3f}')
plt.plot(temp4, QFI_q4, color='red', linestyle='--', label=f'Qubit 4 - TMax = {temp4[np.argmax(QFI_q4)]:.3f}')
plt.plot(temp5, QFI_q5, color='purple', linestyle='--', label=f'Qubit 5 - TMax = {temp5[np.argmax(QFI_q5)]:.3f}')
plt.plot(temp6, QFI_q6, color='yellow', linestyle='--', label=f'Qubit 6 - TMax = {temp6[np.argmax(QFI_q6)]:.3f}')
