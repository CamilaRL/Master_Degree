...
rh, beta_hot, muc, beta_cold = np.loadtxt('./ThermalKinematics/initial_temperatures.txt', unpack=True)
...
for j in range(2):
    ...
    for i in range(2):
        ...
        nome = [r'$\mu_c$ = '+f'{mui[i]:.2f}', r'$r_h$ = '+f'{ri[i]:.2f}']

        r = np.array([mui[i], mui[i]])
