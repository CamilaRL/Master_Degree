...
def ThermalKinematics(mu, beta_i, beta_f, w, gamma, tlist):
    ...
    return Iw_p, Iw_e, Sprod_p, Sprod_e
...
def WriteOutput(mu, processo, tlist, Iw_p, Iw_e, Sprod_p, Sprod_e):        

    f = open(f'./ThermalKinematics_Contributions/mu{mu}-{processo}-contributions.txt', 'w')
    
    for i in range(len(tlist)):
    
        f.write(f'{tlist[i]} {Iw_p[i]} {Iw_e[i]} {Sprod_p[i]} {Sprod_e[i]}\n')
...
Iw_p_heating, Iw_e_heating, Sprod_p_heating, Sprod_e_heating = ThermalKinematics(mu, beta_cold, beta_eq, w, gamma, tlist)
Iw_p_cooling, Iw_e_cooling, Sprod_p_cooling, Sprod_e_cooling = ThermalKinematics(mu, beta_hot, beta_eq, w, gamma, tlist)


## Write Output Files

WriteOutput(mu, 'heating', tlist, Iw_p_heating, Iw_e_heating, Sprod_p_heating, Sprod_e_heating)
WriteOutput(mu, 'cooling', tlist, Iw_p_cooling, Iw_e_cooling, Sprod_p_cooling, Sprod_e_cooling)
