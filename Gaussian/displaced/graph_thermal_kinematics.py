import numpy as np
import matplotlib.pyplot as plt


muList = [0.1, 0.5, 1.0, 2.0]
symbols = ['-', '--', '-.', ':']

## Leitura Arquivos

Iw_list = []
Vw_list = []
Lw_list = []
completion_list = []
Kevol_list = []
Sprod_list = []

for k, mu in enumerate(muList):
    
    tlist, Iw, Vw, Lw, completion, Kevol, Sprod = np.loadtxt(f'./ThermalKinematics/mu{mu}-heating.txt', unpack=True)
    
    Iw_list.append([Iw])
    Vw_list.append([Vw])
    Lw_list.append([Lw])
    completion_list.append([completion])
    Kevol_list.append([Kevol])
    Sprod_list.append([Sprod])
    
    tlist, Iw, Vw, Lw, completion, Kevol, Sprod = np.loadtxt(f'./ThermalKinematics/mu{mu}-cooling.txt', unpack=True)
    
    Iw_list[k].append(Iw)
    Vw_list[k].append(Vw)
    Lw_list[k].append(Lw)
    completion_list[k].append(completion)
    Kevol_list[k].append(Kevol)
    Sprod_list[k].append(Sprod)


## Plots

### Wigner Fisher Information

plt.figure(figsize=(10,5))

for i in range(len(muList)):
    
    plt.subplot(121)
    plt.plot(tlist, Iw_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')
    
    plt.subplot(122)
    plt.plot(tlist, Iw_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')

plt.subplot(121)
plt.xscale('log')
plt.ylabel('Wigner Fisher Information', fontsize=14)
plt.xlabel('Time', fontsize=14)
plt.title('Heating', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.subplot(122)
plt.xscale('log')
plt.xlabel('Time', fontsize=14)
plt.title('Cooling', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.tight_layout()
plt.show()

### Velocity

plt.figure(figsize=(10,5))

for i in range(len(muList)):
    
    plt.subplot(121)
    plt.plot(tlist, Vw_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')
    
    plt.subplot(122)
    plt.plot(tlist, Vw_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')

plt.subplot(121)
plt.xscale('log')
plt.ylabel('Statistical Velocity', fontsize=14)
plt.xlabel('Time', fontsize=14)
plt.title(r'$W_c$', fontsize=16)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.subplot(122)
plt.xscale('log')
plt.xlabel('Time', fontsize=14)
plt.title(r'$W_h$', fontsize=16)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14, loc='upper right')

plt.tight_layout()
plt.show()

### Distance

plt.figure(figsize=(10,5))

for i in range(len(muList)):
    
    plt.subplot(121)
    plt.plot(tlist, Lw_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')
    
    plt.subplot(122)
    plt.plot(tlist, Lw_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')

plt.subplot(121)
plt.ylabel('Statistical Distance', fontsize=14)
plt.xlabel('Time', fontsize=14)
plt.title('Heating', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.subplot(122)
plt.xlabel('Time', fontsize=14)
plt.title('Cooling', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.tight_layout()
plt.show()


### Completion

plt.figure(figsize=(15,5))

for i in range(len(muList)):
    
    num = 141 + i
    
    plt.subplot(num)
    plt.plot(tlist, completion_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$W_c$')
    plt.plot(tlist, completion_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$W_h$')
    
    plt.title(r'$\mu$ = '+f'{muList[i]}', fontsize=16)
    plt.xlabel('Time', fontsize=14)
    plt.xscale('log')
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.legend(fontsize=14)
    
    if i == 0:
        plt.ylabel('Degree of Completion', fontsize=14)

plt.tight_layout()
plt.show()

### Relative Entropy

plt.figure(figsize=(10,5))

for i in range(len(muList)):
    
    plt.subplot(121)
    plt.plot(tlist, Kevol_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')
    
    plt.subplot(122)
    plt.plot(tlist, Kevol_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')

plt.subplot(121)
plt.xscale('log')
plt.ylabel('Relative Entropy', fontsize=14)
plt.xlabel('Time', fontsize=14)
plt.title(r'$W_c$', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.subplot(122)
plt.xscale('log')
plt.xlabel('Time', fontsize=14)
plt.title(r'$W_h$', fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.tight_layout()
plt.show()

### Entropy Production

plt.figure(figsize=(10,5))

for i in range(len(muList)):
    
    plt.subplot(121)
    plt.plot(tlist, Sprod_list[i][0], color='red', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')
    
    plt.subplot(122)
    plt.plot(tlist, Sprod_list[i][1], color='blue', linestyle=symbols[i], linewidth=2, label=r'$\mu$ = '+f'{muList[i]}')

plt.subplot(121)
plt.xscale('log')
plt.ylim(top=0.4)
plt.ylabel('Entropy Production', fontsize=14)
plt.xlabel('Time', fontsize=14)
plt.title('Heating', fontsize=16)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=14)

plt.subplot(122)
plt.xscale('log')
plt.ylim(top=0.4)
plt.xlabel('Time', fontsize=14)
plt.title('Cooling', fontsize=16)
plt.xticks(fontsize=14)
plt.yticks([])
plt.legend(fontsize=14)

plt.tight_layout()
plt.show()






