...
tempo_real = np.arange(0.005, 2, 0.001)
...
for arquivo in os.listdir(f'./DensityMatrices/{modo}/c_{cmod[0]}'):
...
for c in cmod:
    ...
    for t in tempo_index:
        ...
        rhot = np.loadtxt(f'./DensityMatrices/{modo}/c_{c}/{rho_name}{t}.txt', dtype='complex', unpack=True)
