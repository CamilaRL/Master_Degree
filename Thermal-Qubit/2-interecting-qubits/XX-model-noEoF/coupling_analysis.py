...
plt.plot(gList, Tf_list, color='orange', label='Final Temperature')
plt.plot(gList, Tc_list, color='Blue', label='Colder Initial Temperature')
plt.plot(gList, Th_list, color='Red', label='Hotter Initial Temperature')
...
plt.plot(gList, alpha_list, color='black')
plt.ylabel(r'$\alpha$')
plt.xlabel('g')
plt.show()'''

w0 = 1
betalist = np.arange(0.1, 1, 0.1)
g = 0.8
...
for beta in betalist:
    H = -(w0/2)*(tensor(sigmaz(), qeye(2)) + tensor(qeye(2), sigmaz())) + g*(sxsx + sysy)

    evals_H = H.eigenenergies()
    ...
    rho = [[np.exp(beta*w0/2)/Z, 0, 0, 0], [0, np.cosh(beta*g/2)/Z, -np.sinh(beta*g/2)/Z, 0], [0, -np.sinh(beta*g/2)/Z, np.cosh(beta*g/2)/Z, 0], [0, 0, 0, np.exp(-beta*w0/2)/Z]]
