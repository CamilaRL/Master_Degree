...
def Master_Equation(w0, beta_1, beta_2, tlist, L_operators, alpha):
    ...
    entanglement_matrix = alpha * v00 * v11.dag() + alpha.conjugate() * v11 * v00.dag()
...
plt.title(f'XXX Model', fontsize=14)
...
falpha = open('./DensityMatrices/correlations.txt', 'a')
falpha.write(f'{J} {alpha_min_max[0]} {alpha_min_max[1]}\n')
falpha.close()
...
for n, alpha in enumerate(alpha_min_max):
    ...
    Write_Density_Matrices(rhof, rhof_q1, rhof_q2, alphaName[n], J)
