...
def WriteOutput(tlist, velocity, position, completion, i, modo):
    
    
    path = f'./ThermalKinematics_{modo}/'
...
os.mkdir(f'./ThermalKinematics_{modo}')

cmap = plt.get_cmap('rainbow')
colors = iter(cmap(np.linspace(0.1, 1, 5)))#len(curvas))))
...
for i, j in enumerate(curvas):
    ...
    curve_path = f'./FisherInformation_{modo}/QFI_{qubit}_c0.txt'
    ...
    plt.scatter(tlist, vlist, color=cor, s=1)
    plt.xlabel('Time')
    plt.ylabel('Velocity')
    
    plt.subplot(132)
    #plt.scatter(tlist, Llist, color=cor, s=1, label=f'|c| = {cmod[i]:.3f}')
    plt.scatter(tlist, Llist, color=cor, s=1)
    plt.xlabel('Time')
    plt.ylabel('Position')

    plt.subplot(133)
    #plt.plot(tlist, degree_completion, color=cor, label=f'|c| = {cmod[i]:.3f}')
    plt.plot(tlist, degree_completion, color=cor)
    plt.xlabel('Time')
    plt.ylabel('Degree of Completion')
    
    WriteOutput(tlist, vlist, Llist, degree_completion, curva, modo)
