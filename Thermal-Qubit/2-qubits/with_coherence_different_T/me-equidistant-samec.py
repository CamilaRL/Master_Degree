...
def Coherences(w0, beta_c, beta_h):

    Zc = 2*np.cosh(beta_c*w0)
    Zh = 2*np.cosh(beta_h*w0)

    alpha_c = np.exp(-w0*(beta_c + beta_c)/2)/(Zc*Zc)
    alpha_h = np.exp(-w0*(beta_h + beta_h)/2)/(Zh*Zh)
    
    alpha_list = []
    mod_list = []
    
    alpha_min = min([alpha_c, alpha_h])
    
    num = np.arange(0, alpha_min, 0.00001)
    ...
    for r in num:
        ...
        for i in num:
            ...
            if (c_abs <= alpha_c) and (c_abs <= alpha_h) and (c_abs not in mod_list):
...
for a, alpha in enumerate(alpha_list):
    ...
    rhof_heating, rhof_heating_q1, rhof_heating_q2 = Master_Equation(w0, beta_c, beta_c, tlist, L_operators, alpha)

    rhof_cooling, rhof_cooling_q1, rhof_cooling_q2 = Master_Equation(w0, beta_h, beta_h, tlist, L_operators, alpha)
