...
modo = 'Cooling'
pasta = f'./DensityMatrices_c0_{modo}'
...
for arquivo in os.listdir(pasta):
    ...
    if rho_name in arquivo:
        ...
        tempo.append(int(arquivo.replace(rho_name, '').replace('.txt', '')))
...
for t in tempo:
    ...
    rhot = np.loadtxt(f'{pasta}/{rho_name}{t}.txt', dtype='complex', unpack=True)
...
tempo_list = np.arange(0.005, 2, 0.001)

tempo, QFI = Quantum_Fisher_Information(rhot_list, tempo_list, [[2],[2]])


## save QFI in file

os.mkdir(f'./FisherInformation_{modo}')

f = open(f'./FisherInformation_{modo}/QFI_{qubit}_c0.txt', 'w')
