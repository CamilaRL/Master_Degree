...
\begin{figure}[t]
    ...
    \caption{\small Illustration of a single-mode thermal state, displaced coherent state, and squeezed coherent state in the phase space.}
     \vspace{0.1cm}
...
\noindent where we used $\int W(\mathbf{x}) d^2 \mathbf{x} = 1$. Similar to what we did in Eq. \eqref{eq:energia-krissia}, we will introduce the Wigner entropy \cite{landi-2017, gerardo-2014, medina-2025, serafini-book} defined as
\begin{equation}
...
\noindent where we can identify the Wigner relative entropy, that is defined as \cite{landi-2017, medina-2025}
\begin{equation}
   \begin{split}
...
\noindent where $W_\pi(\mathbf{x})$ is the Wigner function of the passive state. The passive state of a Gaussian state is always thermal with inverse temperature $\beta_\pi$ defined by $f(\beta_\pi) = \sqrt{|\Theta|}$. For this reason, we can use $W_{th}=W_\pi$ such that ergotropy becomes \cite{medina-2025}
\begin{equation}
    \mathcal{E}(W(\mathbf{x})) = \omega f(\beta_\pi) K[W(\mathbf{x}) || W_{\pi}(\mathbf{x})].
    \label{eq:ergotropy-gaussian-passive}
\end{equation}

\noindent From this expression, we find that every squeezed and displaced state have ergotropy meaning they are in the nonequilibrium regime \cite{huber-2016}. An interesting result from Ref. \cite{medina-2025} is that squeezed states with higher initial ergotropy relax faster than displaced state, what has been named ergotropic Mpemba effect. It is also shown that, even when the initial ergotropy is the same, the squeezed state also relax faster than the displaced state. These results inspired the study of how displacement and squeezing may affect the thermal relaxation asymmetry.

Given that the thermal state in Eq. \eqref{eq:ergotropy-gaussian-full} is arbitrary, we can assume it is the equilibrium state, $W_{th} = W_{\text{eq}}$, instead of the passive state. Then, making the time derivative Eq. \eqref{eq:ergotropy-gaussian-full}, we find that the total entropy production for Gaussian states have a passive and an ergotropic part as
\begin{equation}
    \begin{split}
...
Now that we briefly redefined some thermodynamic quantities for Gaussian states, we will discuss how the thermal kinematics change in this regime. As discussed in Sec. \ref{sec:thermal-kinematics}, Fisher information is the essential quantity for the thermal kinematics approach of heating-cooling asymmetry. To find a Fisher information for Gaussian states, we will follow the same procedure done in Sec. \ref{sec:thermal-kinematics} to find the Wigner relative entropy as
\begin{equation}
    K[W(\mathbf{x}, \theta+d\theta) || W(\mathbf{x}, \theta)] = \frac{1}{2} \mathcal{I}_W(\theta) d\theta^2 + \mathcal{O}(d\theta^3),
\end{equation}

\noindent where we identify $\mathcal{I}_W(\theta)$ as the Wigner-Fisher information. The details of this calculation can be found in Appx. \ref{apx:WFI}, where we also show that the Wigner-Fisher information can be written in terms of the mean vector and the covariance matrix as
\begin{equation}
    \mathcal{I}_W (t) = \dot{\mathbf{v}}_t^\dagger \Theta^{-1}_t \dot{\mathbf{v}}_t + \text{Tr}[(\Theta_t^{-1} \dot{\Theta}_t)^2].
    \label{eq:wigner-fisher}
\end{equation}

\noindent Note that here we changed $\theta=t$ because in thermal kinematics the interesting parameter is time, and, as a result, the thermal kinematics variables become 
\begin{align}
    &v_W(t) =  \sqrt{\frac{1}{2}\mathcal{I}_W ({\rho}(t))} ,\label{eq:velocidade-gaussian}\\
    &\mathcal{L}_W(t) = \int_0^{t} \sqrt{\frac{1}{2}\mathcal{I}_W ({\rho}(\tau))} d\tau,\label{eq:comprimento-gaussian}\\
    &\varphi_W(t_s) = \frac{\mathcal{L}_W(t_s)}{\mathcal{L}_W(t_f)}. \label{eq:completion-gaussian}
\end{align}

\noindent Additionally, the Wigner-Fisher information can be separated into a passive and ergotropic contribution,
\begin{equation}
    \mathcal{I}_W (t) = \mathcal{I}_\pi (t) + \mathcal{I}_\mathcal{E} (t)
\end{equation}

\noindent To see this, we will use that the passive state of any Gaussian state is always thermal, so the first term of Eq. \eqref{eq:wigner-fisher} is zero because $\mathbf{v}_\pi = 0$. Then, using that the covariance matrix is $\Theta_\pi = f(\beta_\pi) \mathbb{I}$ where $f(\beta_\pi) = \sqrt{|\Theta|}$, we find $\Theta_\pi^{-1} = (1/f(\beta_\pi))\mathbb{I}$ and $\dot{\Theta}_\pi = f(\beta_\pi) \dot{S}_W(t) \mathbb{I}$. Substituting these matrices in the second term of Eq. \eqref{eq:wigner-fisher}, we obtain the passive contribution in the Wigner-Fisher information is
\begin{equation}
    \begin{split}
        ...
        &= 2 \dot{S}_W^2 (t).
    \end{split}
    \label{eq:passive-wigner-fisher}
\end{equation}

\noindent An interesting observation about the time derivative of the Wigner entropy is its relation with the energy change of the passive state. To see this, we use that $E(\pi) = \omega f(\beta_\pi)$ and find
\begin{equation}
    \dot{S}_W = \frac{1}{2} \frac{\partial}{\partial t} \ln (|\Theta_t|) = \frac{\dot{E}(\pi)}{E(\pi)}.
\end{equation}
...
\begin{equation}
    \mathcal{I}_{W_{th}} (t) = 2 \left[ \frac{\partial}{\partial t} \ln \Delta_\beta \right]^2 = 2 \left[ \frac{\dot{\Delta}_\beta}{\Delta_\beta} \right]^2,
    \label{eq:wigner-fisher-thermal}
\end{equation}
...
\begin{figure}[t]
    ...
    \caption{\small Entropy production rate for heating (red) and cooling (blue) thermal states.}
     \vspace{0.1cm}
    {\small Source: By the author}
...
\noindent where $\dot{S}_{W_{th}} = \dot{\Delta}_\beta/\Delta_\beta $ is the time derivative of the Wigner entropy (Eq. \eqref{eq:wigner-entropy}). From this expression, we obtain Fig. \ref{fig:sprod-thermal} by using $K[W_c || W_f] = K[W_h || W_f] = 0.5$ and $T_f = 2$ (in natural units). Note that heating has an initially higher entropy production rate that reaches zero before cooling. This results corroborate with previous studies showing that heating is faster than cooling for Gaussian states far-from-equilibrium \cite{tejero-2025, tejero-misc-2025, ibanez-2024}.
...
\noindent The term containing the displacement parameter ($\mu$) is independent of the system's initial temperature, which means that, applying a displacement operator to any thermal state results in the same shift in their Wigner relative entropy. Additionally, given that the ergotropy of a displaced state is $\mathcal{E}_d= |\mu|^2 e^{-\Gamma t}$, using the same $\mu$ is enough to guarantee different states have the same initial ergotropy and relative entropy. 

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{Images/Gaussian/completion-displaced.png}
    \caption{\small Degree of completion for a displaced thermal state. Each graph corresponds to a different displacement parameter ($\mu$).}
     \vspace{0.1cm}
    {\small Source: By the author}
...
\begin{figure}[b!]
    ...
    \caption{\small Statistical velocity of a displaced thermal state as a function of time. Each curve corresponds to a different displacement parameter ($\mu$).}
     \vspace{0.1cm}
    {\small Source: By the author}
    \label{fig:velocity-displaced}
\end{figure}

The Wigner-Fisher information for a displaced state is
\begin{equation}
    \mathcal{I}_{W_d} (t) =  2 \left[ \frac{\dot{\Delta}_\beta}{\Delta_\beta} \right]^2 + \frac{2}{\Delta_\beta} \left(\omega^2 + \frac{\Gamma^2}{4} \right)|\mu|^2 e^{-\Gamma t},
    \label{eq:WFI-displaced}
\end{equation}

\noindent where we identify the first term as the passive contribution, which is the same as the Wigner-Fisher information of the thermal state presented in Eq. \eqref{eq:wigner-fisher-thermal}. The second term can be identified as the ergotropic contribution. Even though displaced states do not have a real temperature, its passive state is always thermal so we can assume it will heat up or cool down. One interesting aspect of this separation is that the displacement do not affect the trajectory of the passive state, so the heating-cooling asymmetry will not be affected, as we will see in our results.

As we did for the thermal state, we could use Eq. \eqref{eq:WFI-displaced} to try to analytically calculate the thermal kinematics variables. Despite being possible to analytically find the statistical velocity and length, the complexity of the final expressions ultimately makes it difficult to extract useful information. Therefore, we use a numerical approach for this case. The chosen parameter were $\omega = 1$, $\Gamma = 0.1$, $T_f = 2$, and $K[W_c || W_f] = K[W_h || W_f] = 0.5$. Fig. \ref{fig:completion-displaced} shows the degree of completion for different displacement parameters, with the thermal relaxation of $W_c$ (in red) being faster than that of $W_h$ (in blue) for all the displacements. An interesting result is that increasing the displacement parameter, moves the initial state further away from the equilibrium (as one can see from Eq. \eqref{eq:K-displaced}), but the heating-cooling asymmetry remains unchanged. However, the initial statistical velocity also increase with $\mu$ as shown in Fig. \ref{fig:velocity-displaced} and expected from Eq. \eqref{eq:WFI-displaced}. Therefore, a possible explanation for the asymmetry remaining unchanged with displacement is that the initial distance from equilibrium is compensated by a higher initial velocity.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{Images/Gaussian/Sprod_contributions_displaced.png}
    \caption{\small Total entropy production rate(solid) with its ergotropic (dotted) and passive (dashed) contributions for heating (red) and cooling (blue) equidistant displaced thermal states.}
     \vspace{0.1cm}
    {\small Source: By the author}
...
\noindent where $\phi_t = \phi_s - 2\omega t$ is the squeezing phase, the squeezing parameter is implicitly defined as
\begin{equation}
...
\noindent As the squeezing operation affects the thermal occupation number, applying the same squeezing to different thermal states will not preserve the equidistance between them. Just like displacement, squeezing also generates ergotropy in the state, so it is important to choose initial states with same ergotropy. The ergotropy of a squeezed thermal state is
\begin{equation}
    \mathcal{E}_s(t) = \omega f(\beta_t) (\cosh (2r_t) - 1),
    \label{eq:ergo-squeezing}
\end{equation}

\noindent from which we can see that applying the same squeezing to different thermal states will also not result in the same ergotropy. As a consequence, to make a fair comparison between heating and cooling, we must use different squeezing parameters for the colder and the hotter thermal states such that the final squeezed states have the same initial relative entropy from equilibrium and same initial ergotropy. Just like in the displacement case, squeezed states do not have a real temperature, but we can assume it will heat up or cool down because its passive state is always thermal. So, in these results, we represent the initial hotter state as $W_h$ and the initial colder state as $W_c$.

\begin{figure}[b!]
    \centering
    \includegraphics[width=\linewidth]{Images/Gaussian/initialization-squeezing.png}
    \caption{\small Wigner relative entropy of a squeezed thermal state and the equilibrium thermal state as a function of inverse temperature. The black line indicates the chosen initial Wigner relative entropy that both hot (red dot) and cold (blue dot) squeezed states must have. The left plot uses a small squeezing parameters and the right plot use bigger squeezing parameters.}
     \vspace{0.1cm}
    {\small Source: By the author}
    \label{fig:initial-squeezing}
\end{figure}

For clarity, we will present an example of how to find the appropriate initial states. To start, we choose an initial relative entropy $K_0$ and a squeezing parameter, $r_h$, that will be applied in the hotter thermal state, for example. To find the initial temperature $T_h$ of this thermal state, we use that the final squeezed state $W_h$ must satisfy $K[W_h || W_f] = K_0$, which can be numerically done. After finding our hotter initial state, we can try to find the colder initial state. By combining Eq. \eqref{eq:relative-entropy-squeezing} and Eq. \eqref{eq:ergo-squeezing}, we get that $f(\beta_c) - f(\beta_h) + f(\beta_f)\ln(f(\beta_h)/f(\beta_c))=0$ must be true to satisfy the condition of same initial relative entropy and ergotropy. Given that we already know $T_h$, we can solve this equation to obtain the temperature of the cold squeezed state and use Eq. \eqref{eq:coshrt-squeezing} to find $r_c$. Fig. \ref{fig:initial-squeezing} presents Wigner relative entropy as a function of temperature and shows which initial temperatures we find in the end of this process. The left plot corresponds to the smaller squeezing parameters and the right plot corresponds to the bigger squeezing parameters. Note that after increasing $r_h$ by $0.4$, the initial temperature of the hot state drops by half, while increasing $r_c$ by $0.38$ results in a colder temperature four times bigger. This means that the colder states are more sensitive to variations of the squeezing parameter.

The Wigner-Fisher information of a squeezed thermal state is
\begin{equation}
    \mathcal{I}_{W_s} (t) = 2 \dot{S}_{W_s}^2 + 8 \dot{r}_t^2 + 2 \dot{\phi}_t^2 \sinh^2 (2r_t),
...
\begin{figure}[t]
    ...
    \caption{\small Degree of completion for squeezed thermal states (left) and their passive states (right). We compared a smaller squeezing parameter (solid lines) with a bigger squeezing parameter (dashed lines) for both initially colder ($r_c$) and hotter ($r_h$) states.}
     \vspace{0.1cm}
    {\small Source: By the author}
    \label{fig:completion-squeezing}
\end{figure}

\noindent Again, obtaining the analytical thermal kinematic expressions from Eq. \eqref{eq:Iw-squeezing} will not bring additional information, so we will solve it numerically. In Fig. \ref{fig:completion-squeezing}, we show the degree of completion as a function of time for the squeezed thermal state (left) and its passive state (right). In each plot, we compared a squeezing with small parameters (solid lines) with a squeezing with bigger parameters (dashed lines). In both plots, because $W_c$ (in red) is always above $W_h$ (in blue), we can say that the initially colder state relaxate faster than the initially hotter state. In the left plot, by comparing the solid lines with the dashed lines, we observe that increasing the squeezing parameters reduces the heating-cooling asymmetry of the total state, while in the right plot, the asymmetry in the passive states seems to increase as we increase the squeezing parameters.
...
\begin{figure}[b!]
    ...
    \caption{\small Entropy production rate for heating (red) and cooling (blue) squeezed thermal states with smaller parameters (upper plots) and bigger parameters (lower plots). We separate the passive contribution (dashed) and ergotropic contribution (dotted) of the total entropy production rate (solid). The inset plot show the region where the passive state entropy production becomes negative.}
     \vspace{0.1cm}
    {\small Source: By the author}
    \label{fig:contribution-squeezing}
\end{figure}

Fig. \ref{fig:contribution-squeezing} show the entropy production rate, where the upper plots correspond to smaller $r$ and the lower plots correspond to bigger $r$. Each graph shows the total entropy production rate (solid line) with its passive (dashed line) and ergotropic (dotted line) contributions. First, the initial total entropy production of $W_c$ is bigger than the initial total entropy production of $W_h$, which explain why heating is faster than cooling again. Also, when we increase the squeezing parameters, the heating and the cooling total entropy productions decrease, but the heating process suffers a bigger reduction when compared with cooling. This can be explained by the previous discussion that colder states have higher sensitivity to squeezing variations. As a result, for bigger squeezing parameters, the difference in initial total entropy production between heating and cooling is smaller, which explains why the asymmetry was reduced for the total squeezed state. As we already noticed in the equations of entropy production, unlike the displacement operation, squeezing affects both passive and ergotropic contributions, which means that the asymmetry is not fully generated by the passive state. When we look at the passive contribution, the difference between the initial entropy production of heating and cooling becomes bigger as we increase the squeezing parameter. This can explain why the asymmetry in the passive contribution increases. The explanation of how these related quantities can have opposite behaviours with the increase of squeezing parameter is in the ergotropic contribution. Note in Fig. \ref{fig:contribution-squeezing} that the ergotropic contribution increase with $r$, but it is not enough to compensate the decrease of the passive contribution, resulting in the reduction of the asymmetry.

One last comment of this section is regarding the oscillation in the degree of completion that appears for the passive state of the big squeezing parameter case (red dashed line of the right plot in Fig. \ref{fig:completion-squeezing}). Additionally, the statistical velocity of this same passive state is shown in Fig. \ref{fig:velocity-squeezing-passive} and also presents this oscillation. Recalling the degree of completion and statistical velocity we had for the interacting qubits, we also saw this oscillations and attributed them to a local non-Markovian dynamics witnessed as negative entropy production. The inset in Fig. \ref{fig:contribution-squeezing} shows that the entropy production of the passive state with big squeezing parameter also have negative value for a period of time, which is a signature of non-Markovianity. From this observations, we can again affirm that non-Markovianity may be related to faster relaxation. The origin of this non-Markovian dynamics is a consequence of squeezing affecting the passive state, whose time evolution will not be described by Eq. \eqref{eq:lindblad-gaussian}. It is interesting that negative entropy production is only observed for heating state with big squeezing parameter and not in its correspondent cooling state. One possibility is that the squeezing parameter used in heating is bigger than the one in cooling. However, this cooling process do have a different behaviour, which is a small increase in its entropy production and statistical velocity. This means that, instead of receiving a feedback information from the environment, the system accelerates and loose more information to the environment.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/Gaussian/Vw-squeezing-passive.png}
    \caption{\small Statistical velocity of the heating (red) and cooling (blue) passive states of squeezed states with smaller parameter (solid lines) and bigger parameters (dashed lines).}
     \vspace{0.1cm}
    {\small Source: By the author}
...
\begin{figure}[b!]
    ...
    \caption{\small Degree of completion of the passive state of the heating displaced state (red) and of the cooling squeezed state (blue).}
     \vspace{0.1cm}
    {\small Source: By the author}
    \label{fig:completion-inversion-passive}
\end{figure}

To study the entropy production rate of heating the displaced state, we used the same expressions of Section \ref{sec:result-displaced} and, for the entropy production rate of cooling the squeezed state, we used the expressions of Section \ref{sec:result-squeezed}. Fig. \ref{fig:sprod-inversion} shows the total entropy production (solid lines), the passive (dashed lines) and the ergotropic (dotted lines) contributions for the entropy production of smaller and bigger parameters, respectively. Given that heating is faster for the passive states, what accelerates cooling to create the inversion must be the ergotropic contribution. Different from the results comparing only squeezing or only displacement, the ergotropic contribution of cooling (squeezing) is initially higher than the one of heating, which explains the full inversion of the asymmetry. Furthermore, for the partial inversion, we see that the ergotropic part has almost no participation in the relaxation and all the asymmetry was generated by the passive state. In this case, heating has higher initial entropy production and is faster than cooling according to the degree of completion in Fig. \ref{fig:completion-inversion}. When the entropy production of cooling cross with heating, we find the partial inversion in the degree of completion. In this case, the initial entropy production could not tell us which process relaxate faster, but its time evolution is still helpful to understand the dynamics. Heating crossing cooling at some time can be a consequence of comparing the relaxation of a displaced state with a squeezed state, as it was similarly observed in the ergotropic Mpemba effect \cite{medina-2025}. 


\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{Images/Gaussian/Sprod-inversion-all.png}
    \caption{\small Left plot shows the entropy production rate of smaller $\mu$ and $r$. Right plot shows the entropy production rate of bigger $\mu$ and $r$. Heating the displaced state is in red and cooling the squeezed state is in blue. The passive contribution is in dashed lines, the ergotropic contribution is in dotted lines, and the total entropy production rate is in solid lines.}
     \vspace{0.1cm}
    {\small Source: By the author}
